...
function getInitialGeoPoint(models: LoadedGeoModel[], referenceCenter: THREE.Vector3): THREE.Vector3 {
  return models.reduce((bestPoint, model) => {
    if (!bestPoint) {
      return model.sourceReferencePoint.clone();
    }

    const currentDistanceSq = bestPoint.distanceToSquared(referenceCenter);
    const candidateDistanceSq = model.sourceReferencePoint.distanceToSquared(referenceCenter);

    return candidateDistanceSq < currentDistanceSq
      ? model.sourceReferencePoint.clone()
      : bestPoint;
  }, null as THREE.Vector3 | null) ?? referenceCenter.clone();
}

function getMarkerScale(): number {
  const sceneBox = new THREE.Box3().setFromObject(modelsGroup);
  const size = sceneBox.getSize(new THREE.Vector3());
  const diagonal = size.length();

  return Math.max(3, diagonal * 0.012);
}
...
async function init(): Promise<void> {
  try {
    ...
    const initialGeoPoint = getInitialGeoPoint(loadedModels, globalOrigin);

    inputX.value = initialGeoPoint.x.toFixed(4);
    inputY.value = initialGeoPoint.y.toFixed(4);
    inputZ.value = initialGeoPoint.z.toFixed(4);
    ...
    createMarkerAtSourceCoords(initialGeoPoint.x, initialGeoPoint.y, initialGeoPoint.z);

    statusEl.textContent =
      'Modelos cargados correctamente. El valor inicial corresponde a un punto real de la geometría.';
  } catch (error) {
    console.error(error);
...
function createMarkerAtSourceCoords(x: number, y: number, z: number): void {
  ...
  const sphereRadius = markerScale * 0.14;
  const crossRadius = markerScale * 0.45;

  const markerGroup = new THREE.Group();
  markerGroup.position.copy(scenePosition);
  ...
  const localAxes = new THREE.AxesHelper(markerScale * 0.55);
  ...
  statusEl.textContent =
    `Marcador creado en coordenadas reales (${x.toFixed(4)}, ${y.toFixed(4)}, ${z.toFixed(4)}) → ` +
